...
export default function Navbar() {
  ...
  return (
    <nav className="w-full flex justify-center bg-[#F7F7F7] fixed top-0 left-0 z-50 shadow-sm">
      ...
      {/* Contenedor Maestro FLUIDO (Liquid Design) */}
      {/* Le sacamos el max-w-[1440px] para que ocupe el 100% real.
        El padding lg:px-24 asegura que se alinee perfecto con la sección de abajo.
      */}
      <div className="w-full px-4 md:px-8 lg:px-24 flex justify-between items-center h-[90px]">
        ...
        {/* 2. Links de Navegación (Se quedan igual) */}
        ...
        {/* 3. Botón de Acción y Lenguaje */}
        <div className="flex items-center gap-4">
          <LanguageSwitcher />
          <Link 
          href="/contacto" 
          className="border-2 border-[#18181A] px-8 py-2 rounded-lg text-sm font-medium text-[#18181A] hover:bg-[#60A572] hover:border-[#60A572] hover:text-white transition-all duration-300"
          >          
          {t('btn_contacto')}
        </Link>
        </div>
      ...
      {/* MENÚ DESPLEGABLE MOBILE (Intacto) */}
      {isMobileMenuOpen && (
        <div className="fixed top-[90px] left-0 w-full h-[calc(100vh-90px)] bg-[#F7F7F7] z-[55] flex flex-col pt-8 px-8 md:hidden overflow-y-auto overflow-x-hidden">
          ...
          <div className="mt-auto pb-12 flex flex-col gap-6">
            ...
            <Link href="/contacto" onClick={() => setIsMobileMenuOpen(false)} className="w-full">
